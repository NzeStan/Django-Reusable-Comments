...
**Authentication**: Not required (public comments only without auth)
...
- `parent` (uuid): Filter by parent comment ID
...
**Example Request**:
```http
GET /api/comments/?content_type=blog.post&object_id=123&ordering=-created_at&page=1
```
...
// 3. Get comments for object
const listResponse = await fetch(
    `/api/content/blog.post/123/comments/?ordering=-created_at`
);
...
const api = axios.create({
    baseURL: 'http://example.com/api/comments/',
    headers: {
